...
impl Window {
    ...
    pub fn tick(&mut self) -> bool {
        ...
        if let Some(ref mut i) = self.internals {
            ...
            //window.context().swap_buffers().ok();
        }
        ...
        let (layout_size, framebuffer_size, win_id) = if let Some(ref mut i) = self.internals {
            /*let win_id = unsafe {
                ...
                let _window = i.gl_window;
                let tmp_r = _window.make_current();
                i.gl_window = tmp_r.unwrap();
                /*if tmp_r.is_err() {
                    return false;
                }*/
            ...
            dpi = i.gl_window.window().get_hidpi_factor();
            let framebuffer_size = {
                let size = i.gl_window.window().get_inner_size().unwrap().to_physical(dpi);
                DeviceIntSize::new(size.width as i32, size.height as i32)
                //DeviceUintSize::new(size.width as u32, size.height as u32)
            };
            let layout_size = framebuffer_size.to_f32() / euclid::TypedScale::new(dpi as f32);
